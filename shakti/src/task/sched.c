...
int
sched_create(char *name, uint32_t ds, uint32_t cr3, uint32_t eip,
	     uint32_t esp, uint32_t eax, uint32_t ebx, uint32_t ecx,
	     uint32_t edx)
{
	if (kstrlen(name) > 20) {
		return -1;
	...
	processes[index].eip = eip;
	processes[index].esp = esp;
	processes[index].eax = eax;
	processes[index].ebx = ebx;
	processes[index].ecx = ecx;
	processes[index].edx = edx;
	return 0;
}

int
sched_create_thread(int pid, uint32_t eip, uint32_t esp, uint32_t eax,
